...
public: // Name and guiders
  ...
  const std::string&
  getRequesterName() const
  {
    return m_requesterName;
  }

  Interest&
  setRequesterName(const std::string& name)
  {
    m_requesterName = name;
    m_wire.reset();
    return *this;
  }
...
public: // Selectors
  ...
  /**
   * \brief Set Interest type to Persistent Interest
   * 
   * @param push Persistent Interest if set to True
  **/
  Interest&
  setPush(const bool push);

  /**
   * \brief Returns true in case of an Persistent Interest
  **/
  bool
  isPush() const;
   ...
   * \brief Set QCI class
   * 
   * @param qci QCI value according to ENUM from qci.hpp
  **/
  Interest&
  setQCI(uint32_t qci)
  {
  ...
  /**
   * \brief Returns QCI class of the packet
  **/
  uint32_t
  getQCI() const
  {
