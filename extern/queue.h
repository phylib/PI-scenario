...
public:
  ...
  /**
   * \brief Enumeration of supported Drop-Policies
   *
   */
  enum DropPolicy
  {
    QUEUE_MODE_TAIL_DROP,             /** Drop the last packet in queue (Tail-Drop) */
    QUEUE_MODE_LOWEST_PRIORITY_DROP,  /** Drop the packet with the lowest priority (only possible in ẂeightedFairQueue and PriorityQueue) */
  };
